import Tassis from "../models/Tassis.js";
import PDFDocument from "pdfkit";
import Dossier from "../models/Dossier.js";

export const createTassis = async (req, res) => {
  try {
    const tassis = await Tassis.create(req.body);
    res.status(201).json(tassis);
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
};

export const getTassiss = async (req, res) => {
  try {
    const tassiss = await Tassis.find()
    .populate({
        path: "dossier",
...
export const deleteTassis = async (req, res) => {
  try {
    await Tassis.findByIdAndDelete(req.params.id);
    res.json({ message: "Tassis supprimée" });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
export const generatetassisPdf = async (req, res) => {
  try {
    const tassis = await Tassis.findById(req.params.id)
      .populate({
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename=recu-tassis-${tassis._id}.pdf`
    );
...
const dateTas = new Date(tassis.dateTassis).toLocaleDateString("ar-DZ");
...
if (tassis?.dossier) {
    const { juridictionType, tribunal, classe, court, chambre, numero } = tassis.dossier;
...
doc.moveDown(0.2);
doc.text(`${dateTas || "..."} : ${rtl(" الجلسة تاريخ  ")}`, { align: "right" });
...
doc.moveDown(1);
const lineTop0 = doc.y; // Position actuelle du curseur
doc.moveTo(50, lineTop0)          // Point de départ (x=50, y=courant)
   .lineTo(550, lineTop0)         // Point d'arrivée (x=550 pour couvrir la largeur A4)
   .lineWidth(1)                 // Épaisseur de la ligne
   .strokeColor("#bdc3c7")       // Couleur grise claire (style pro)
   .stroke();                    // Dessiner la ligne

   doc.moveDown(0.4);
const momo3 = tassis.desc || "...";
doc.fontSize(16)   
...
doc.moveDown(0.3);
doc.fontSize(14);
doc.text((" الموضوع:  "), { align: "right", underline: true  });
doc.moveDown(0.2);
doc.fontSize(18);
doc.text(("  تأسيس إعلان  "),  { align: "center", underline: false  });
doc.moveDown(0.8);
//doc.text(`${delegation?.dossier?.client?.noms.replace(/\s+/g, '_') || "..."} `, { align: "right" });
//******************************************************* */
// Préparation des données
doc.fontSize(12);
doc.text((" الرئيس، السيد  "), { align: "right" });
doc.text(( ` :  موكلي  مصالح عن للدفاع وكلت أني علما سيادتكم أحيط أن الشرف لي  ` ), { align: "right" });
doc.fontSize(15);
doc.fillColor("#444444");
doc.text(( `${rtl(momo)}` ), { align: "center" });
doc.fillColor("black"); // Retour au noir
doc.fontSize(12);
doc.text((". أعلاه الهامش على إليها المشار القضية في "),{ align: "right" });
doc.text(("  حقوقه عن الدفاع لي  يتسنى حتى الموضوع ملف على الإطلاع من  تمكيني و بذلك  لي الإشهاد الرجاء  "), { align: "right" });
doc.text((". التقدير و الاحترام فائـق سيدي مني تقبلـوا عليه و  "), { align: "right" });
...
doc.fontSize(12);
doc.text(`${dateTas || "..."} : ${rtl("  في الجزائر  ")}`, { align: "left" });
const lineTop5 = doc.y; // Position actuelle du curseur
doc.moveTo(50, lineTop5)          // Point de départ (x=50, y=courant)
   .lineTo(550, lineTop5)         // Point d'arrivée (x=550 pour couvrir la largeur A4)
   .lineWidth(1)                 // Épaisseur de la ligne
   .strokeColor("#bdc3c7")       // Couleur grise claire (style pro)
   .stroke();                    // Dessiner la ligne
   doc.moveDown(0.5)
   doc.fontSize(12);
   doc.text(("الدفاع محترمكم "), { align: "center" });
